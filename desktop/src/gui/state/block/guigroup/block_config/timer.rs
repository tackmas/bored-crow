...
use iced::{
    ...
    widget::{Column, PickList, Row, Text, TextInput, center},
};
...
impl Timer {
    ...
    pub fn view(&self) -> Element<'_, Message> {
        center(self.timer()).into()
    }

    pub fn timer(&self) -> Element<'_, Message> {
        let selected_time_unit = PickList::new(
            TimeUnit::TIME_UNITS,
            Some(self.time_unit),
        ...
        let text_input = TextInput::new(
            "Enter how long you want to block the group",
            &self.text_input,
        )
        .on_input(Message::TextInputChanged);

        let row = Row::with_children([text_input.into(), selected_time_unit.into()]);

        let column =
            Column::with_children([instruction.into(), row.into()]).align_x(Horizontal::Left);

        column.into()
    }

    pub fn try_duration(&self) -> Result<Duration, &'static str> {
        let parse_result = self.text_input.parse::<u64>();
